...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInst, LPSTR lpCmdLine, INT nCmdShow)
{
	...
	if (hwnd == NULL)
	{
		MessageBox(NULL, "Ошибка функции WindowCreation", NULL, MB_OK | MB_ICONERROR);
		return 0;
	}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		(
			...
			10, 10,
			g_i_DISPAY_WIDTH, g_i_DISPLAY_HEIGHT,
			hwnd,
			(HMENU)IDC_DISPLAY,
	...
	case WM_CONTEXTMENU:
	{
		...
		CHAR sz_digit[2] = {};
		SendMessage(hEdit, WM_GETTEXT, SIZE, (LPARAM)sz_buffer);
		SendMessage(hEdit, WM_SETTEXT, 0, (LPARAM)sz_buffer);				//Отрисовывется окно ввода при смене темы (ноль меняет цвет (красный\зелённый))
	}
	break;
	case WM_DESTROY:
		FreeConsole();
